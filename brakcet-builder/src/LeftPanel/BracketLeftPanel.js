...
function BracketLeftPanel({input}) {
  ...
  const [songs, setSongs] = useState(input)
  ...
  const updateBracket = (currentRound, j) => {
    ...
    let newRounds = [...round]
    newRounds[roundNum]["seeds"][Math.trunc(matchNum / 2)]["teams"][matchNum % 2]["name"] = songs[matchNum * 2 + j];
    if (matchNum >= 3) {
      setRoundNum(2)
      setMatchNum(0)
    } else {
      ...
      let newSongs = [...songs]
      newSongs[matchNum] = songs[matchNum * 2 + j]
      setSongs(newSongs)
    }
  }
  ...
  const [round, setRound] = useState(initialRounds)
    ...
    return <div className="LeftPanel">
      ...
      <button onClick={() => updateBracket(input, 0)}>{songs[matchNum * 2]}</button>
      <button onClick={() => updateBracket(input, 1)}>{songs[matchNum * 2 + 1]}</button>
    </div>
